...
- Server alert engine (rewritten 2026-06-10, extended 2026-06-13) is a snapshot → rules → dispatcher pipeline: one weather + one AQI fetch per device per run (shared per rounded coordinate), future-aware rain probability checks, and short-lived prior snapshots for sudden rain/wind/visibility/temperature/AQI changes. Dedicated rules cover PMD, NDMA, native WeatherKit warnings, motorway subscriptions, broad major closures, storms, heavy/light/future/sudden rain, wind, severe AQI/PM2.5, smog, pollen, heat/cold, fog/visibility drops, good-window, morning brief, and evening planner. Each push is tagged with a decision verdict (`avoid`/`caution`/`go`/`plan`) and severity (`critical`/`important`/`helpful`). Dispatcher enforces quiet hours 22:00–06:00 local, server-side "Mute alerts today", unified cooldowns, 2/day non-critical cap, and max 1 non-critical push per run. **AI-first copy (added 2026-07-04):** every approved push (briefs + alerts) is rewritten by Gemini in `api/_lib/aiNotificationWriter.js` from a grounded evidence bundle (current weather, next-12h hourly, AQI, device-matched PMD/NDMA advisories with validity, trigger data); verdict/severity stay deterministic and are prepended server-side; AI analyses are cached per rounded location + alert type and refreshed `AI_NOTIFY_TIMES_PER_DAY` (default 6) times/day with time-bound expiry (rain arrival hour, ≥12h for static official advisories) and a coarse evidence hash for early refresh; global daily Gemini cap `AI_NOTIFY_DAILY_CALL_CAP` (default 80, criticals bypass); any Gemini failure or missing `GEMINI_API_KEY` ships the rule-based copy unchanged, and `AI_NOTIFY_DISABLED=1` is the kill switch. Criticals send as iOS time-sensitive; non-criticals carry the `oa-alert` actionable category. `DeviceNotRegistered` receipts auto-remove dead tokens; `/api/push?action=status` is a secret-protected delivery dashboard. WeatherKit primary (via `/api/weatherkit`), Open-Meteo fallback. Five-minute Cloudflare Worker scheduler `outdooradvisor-notification-cron` is live; GitHub Actions remains the 15-minute fallback.
