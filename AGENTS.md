...
## Recent Changes
- 2026-06-13 — Made the Home `Pakistan Weather Alerts` section permanently visible. Active PMD alerts remain pinned with no dismiss control; when no advisory is active, the section now shows a compact clear status instead of disappearing, and it distinguishes loading/feed-unavailable states. JS-only and suitable for OTA; no EAS build required.
