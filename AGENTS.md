...
4. TestFlight / App Store Connect: build `18` is visible in App Store Connect and assigned to internal groups `FF` + `TE`, but the Apple UI shows it as `Ready to Submit`. That means it is already uploaded; do not keep rerunning EAS upload blindly. To match build `14` (`Waiting for Review`, groups `FF` + `TE` + `BT`), submit build `18` for Beta App Review / external testing from App Store Connect and paste the `TestFlight What To Test` text from `APP_STORE_METADATA.md`.
