...
- Server alert engine (rewritten 2026-06-10) is a snapshot → rules → dispatcher pipeline: one weather + one AQI fetch per device per run (shared per rounded coordinate), 14 rules (PMD, NDMA, motorway, storm, heavy/light rain, rain-soon, wind, severe AQI, extreme heat, cold snap, local fog, good-window, morning brief, evening planner), each push tagged with a decision verdict (`avoid`/`caution`/`go`/`plan`) and severity (`critical`/`important`/`helpful`). Dispatcher enforces quiet hours 22:00–06:00 local, server-side "Mute alerts today", unified cooldowns, 2/day non-critical cap, and max 1 non-critical push per run. Criticals send as iOS time-sensitive; non-criticals carry the `oa-alert` actionable category. `DeviceNotRegistered` receipts auto-remove dead tokens; `/api/push?action=status` is a secret-protected delivery dashboard. WeatherKit primary (via `/api/weatherkit`), Open-Meteo fallback. Remaining follow-ups: NDMA attachment parsing, pollen + smog-season rules, server-side inbox
