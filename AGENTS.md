...
- Native push backbone now exists, build version `18` was produced from commit `be00c29`, and the production push route now has a valid Expo/EAS APNs key (`ZQ96CMG8QN`). A protected production test push returned an Expo ticket with status `ok`; confirm on-device receipt while the app is closed, then restore recurring scheduler automation for `/api/push?action=cron`.
...
4. Build a new TestFlight preview build to include the TestFlight premium access fix and splash icon fix: `eas build --platform ios --profile preview` from `/Users/ahmedadnan/OutdoorAdvisor-main` after `git pull origin master`. Build `18` predates these changes and does not include them.
...
6. Set `GEMINI_API_KEY` in Vercel env — AI Brief FAB shortcut and SynthesisCard will show full Gemini synthesis once key is set.
