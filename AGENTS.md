...
- Server alert engine (`api/_lib/alertEngine.js`) is a snapshot → rules → dispatcher pipeline: one weather + one AQI fetch per device per run (shared per rounded coordinate), future-aware rain probability checks, and short-lived prior snapshots for sudden rain/wind/visibility/temperature/AQI changes. Dedicated rules cover PMD, NDMA, native WeatherKit warnings, motorway subscriptions, broad major closures, storms, heavy/light/future/sudden rain, wind, severe AQI/PM2.5, smog, pollen, heat/cold, fog/visibility drops, good-window, morning brief, and evening planner. Each push is tagged with a decision verdict (`avoid`/`caution`/`go`/`plan`) and severity (`critical`/`important`/`helpful`). Dispatcher enforces quiet hours 22:00–06:00 local, server-side "Mute alerts today", unified cooldowns, 2/day non-critical cap, and max 1 non-critical push per run. Criticals send as iOS time-sensitive; non-criticals carry the `oa-alert` actionable category. `DeviceNotRegistered` receipts auto-remove dead tokens; `/api/push?action=status` is a secret-protected delivery dashboard (includes an `ai` block, see below). WeatherKit primary (via `/api/weatherkit`), Open-Meteo fallback. Five-minute Cloudflare Worker scheduler `outdooradvisor-notification-cron` is live; GitHub Actions remains the 15-minute fallback.
- **AI-first push copy (2026-07-04):** every push the dispatcher approves is rewritten by Gemini in `api/_lib/aiNotificationWriter.js` from a grounded evidence bundle (current weather, next-12h hourly, AQI, device-matched PMD/NDMA advisories with validity, trigger data) — same grounded-evidence approach as Ask OutdoorAdvisor. The rule engine still owns the verdict/severity/timing; the AI only writes the words, and the server prepends the verdict prefix. Analyses are cached per rounded location + alert type, refreshed `AI_NOTIFY_TIMES_PER_DAY` (default 6) times/day, with time-bound expiry (rain-arrival hour, ≥12h for static official advisories) and a coarse evidence hash that forces an early refresh only when conditions change materially. Global daily Gemini cap `AI_NOTIFY_DAILY_CALL_CAP` (default 80; criticals bypass). Any Gemini failure or missing `GEMINI_API_KEY` ships the deterministic rule copy unchanged; `AI_NOTIFY_DISABLED=1` is the kill switch. Full design: `NOTIFICATION_ARCHITECTURE.md`.
...
## Recent Changes
- 2026-06-29 — Created an App Store-ready promotional/ad screenshot asset at `assets/appstore-screenshots/advertising/01_know_before_you_step_out_1290x2796.png`. It is a 1290×2796 iPhone 6.9-inch PNG composed from the approved light/dark app screenshots with brand-safe copy: `Know before you step out.` No app code, API, OTA, or native build changes.
- 2026-06-25 — Tightened Ask OutdoorAdvisor out-of-scope handling after a previous Murree answer remained visible for `What is 2+2`. The server now only inherits previous destination/activity context for short genuine follow-up wording and force-classifies obvious math/trivia/code/poem-style prompts as out of scope even if prior outdoor context exists. The Ask modal also clears any old result/error as soon as the user edits the input, preventing stale evidence from appearing to answer the next question. JS/API-only; ship via Vercel + OTA, no native EAS build required.
- 2026-06-25 — Changed the app's first-run theme default from Auto to Dark in `src/context/ThemeContext.js`. Existing users with a saved Light/Auto/Dark preference keep their choice; only fresh installs or users without `outdooradvisor_theme_mode` saved now start in Dark. JS-only; ship via OTA, no native EAS build required.
- 2026-06-25 — Reworked Ask OutdoorAdvisor scope and cost controls. `/api/ai/briefing` now defaults Gemini calls to the low-cost `gemini-2.5-flash-lite` model unless `GEMINI_MODEL` explicitly overrides it. Premium Ask requests are limited server-side to 10 per user per Pakistan day using Vercel KV. The old local keyword hard-rejection was removed from the main path: Ask now first uses a small Gemini scope-classifier to decide whether the message belongs inside OutdoorAdvisor (weather, AQI, outdoor activity, nearby places, camping, route stops, NHMP/PMD/NDMA travel safety, follow-ups). In-scope queries continue through the existing evidence/API path; out-of-scope and quota-limit responses return friendly in-app cards without spending route/weather/places evidence fetches. Ask answer budget was expanded slightly so route, camping, and food-stop questions can include more specific evidence. API/JS-only; deploy via Vercel, no native EAS build required.
- 2026-06-25 — Improved rain notification timing at the user pin. Server alert copy now uses the hourly rain signal from WeatherKit/Open-Meteo to include a local estimated arrival time such as `around 5:00 PM` when available, and says `active now at your pin` when heavy rain is already detected at the pin. The `sudden-rain` rule now titles pushes as `Rain may reach <area> soon`, includes the estimated pin timing in the body, and stores `rainAt` / `rainProbability` in push data. Focused alert-engine tests cover the local-time wording. Server/API-only; deploy via Vercel, no OTA or native EAS build required.
- 2026-06-23 — Cleaned up the in-app Notifications center. Notification cards in `HomeScreen` now render flat with card shadows/elevation disabled to remove the grey shadow panel behind alert banners. The header action is now `Clear all` and actually empties the inbox. `notificationInbox` also includes a one-time storage reset (`2026-06-23-clear-old-notifications`) so old saved notifications disappear on the next app launch/update check while new alerts continue saving normally. JS-only; ship via OTA, no native EAS build required.
- 2026-06-23 — Restored the daily Morning Brief and Evening Planner notification system while keeping the stricter major-alert posture. Server alerts now emit cap-exempt once-per-day `morning-brief` and `evening-planner` candidates again when the device is inside the local 6–10 AM / 7–10 PM windows; the morning copy uses local weather/AQI plus national sample context and cached PMD/NDMA official context. Existing devices that received the temporary "major warnings only" OTA get a one-time preference migration so briefs turn back on, but future user opt-outs are respected. Settings → Notifications shows Morning Brief and Evening Planner again. The notification inbox no longer prunes brief/summary cards, but still suppresses routine local Smart movement nudges such as `Avoid the hot window` / `Shift movement indoors`. JS/API-only; ship via Vercel + OTA, no native EAS build required.
- 2026-06-21 — Reduced in-app notification-center noise that survived the server push cleanup. The old local Smart Advisor no longer sends routine movement nudges such as `Shift movement indoors` / `Avoid the hot window`; it still computes health/outdoor score snapshots for the Home card. The notification inbox now prunes already-saved routine Smart/Summary cards while keeping official PMD/NDMA, WeatherKit, heat/smog/AQI, route, and sudden-change alerts. Settings → Notifications now presents only warning-style categories (official advisories, AQI/smog, rain/storm/wind/heat/fog, route closures) and default preferences start routine summaries/good-window/smart-walk/cold nudges disabled. JS-only; ship via OTA, no native build required.
- 2026-06-21 — Fixed several live evidence failures after Google Maps Platform started returning billing/permission errors. Ask OutdoorAdvisor now parses destination-first phrasing such as `Islamabad travel`, `about Islamabad travel`, `I want to travel Islamabad`, and `Islamabad trip tomorrow` instead of falling back to the user's Lahore/current weather. Ask destination geocoding now falls back from Google to OpenStreetMap Nominatim and then a built-in Pakistan destination table. `/api/google/aqi` and server AQI checks now fall back to Open-Meteo Air Quality when Google Air Quality is unavailable, so AQI/PM2.5 no longer disappear solely because Google billing is disabled. `/api/google/geocode` now falls back to OpenStreetMap reverse geocoding, and device location fetching now tries high accuracy, balanced accuracy, then recent last-known GPS before falling back to Lahore. Google Places/Routes still require Google Cloud billing plus Places API (New)/Routes permissions; nearby places remain unavailable until those are restored. JS/API-only; ship via Vercel + OTA, no native build required.
- 2026-06-20 — Reduced server push notification noise per user preference. The alert engine now focuses on official PMD/NDMA/native warnings, thunderstorm, heavy rain, heatwave/extreme heat, severe AQI/smog, critical route closures, and sudden user-area changes such as sharp rain-probability jumps, wind strengthening, visibility drops, heat surges, and AQI spikes. Suppressed routine morning/evening briefs, good-window nudges, pollen, cold-snap, light-rain/rain-soon, routine fog/wind, and non-closure motorway update pushes; non-critical daily cap is now 1. Server/API-only; deploy via Vercel, no native build required.
- 2026-06-20 — Tightened precise-location labeling and thunderstorm scoring. The nearest known AQI sub-area fallback now only labels a device location as a named neighborhood when the GPS point is within 2.5 km of that sample, preventing false labels like `DHA Phase 8` when Google reverse geocoding only returned broad `Lahore`. Health & Outdoor Score now treats WMO 95/96/99 thunderstorms as a hard cap (max 2/10), caps heavy rain and rain, uses feels-like temperature when available, and recomputes when feels-like/wind changes so the score cannot remain high while Live Conditions says Thunderstorm. JS-only; ship via OTA, no native build required.
- 2026-06-17 — Fixed Home sometimes showing only broad city names such as `Lahore` even when precise GPS coordinates were available. Added a shared precise-location helper that upgrades broad city/Pakistan labels to the nearest known sub-area from the AQI area grid when the GPS point is close enough, applies it to fresh and cached device locations, and reuses the same helper for Ask OutdoorAdvisor. Existing cached `Lahore, Pakistan` device labels are corrected on app launch without waiting for cache expiry. JS-only; ship via OTA, no native build required.
- 2026-06-17 — Updated App Store keyword metadata to explicitly include `outdoor` per user request. The keyword field is now `air,quality,pollen,allergy,forecast,UV,Lahore,Islamabad,Karachi,Murree,motorway,traffic,heat,outdoor` (100/100 characters); `fog` was removed from the keyword field to stay within Apple's limit, while fog remains represented in app copy/screenshots/features. Metadata/docs only; no build or deploy required.
- 2026-06-17 — Added route-stop planning to premium Ask OutdoorAdvisor. Questions such as “I want to go to Murree, tell me the best place to eat midway” now classify as a dedicated `route_stop` intent, normalize the common “Muree” typo to Murree, fetch the Google driving route, choose the route midpoint, and search Google Places for food/cafe/dhaba options around that corridor point. Fallback and Gemini prompting now require named midpoint stop options plus relevant NHMP route clarity instead of generic weather/trip copy. API-only; no native build required.
- 2026-06-17 — Fixed Ask OutdoorAdvisor rejecting valid “where to play tennis now” style queries. Expanded the server-side outdoor/activity vocabulary to include tennis, padel, badminton, basketball, yoga, and gym/fitness terms, plus court/club nearby-place language, so these questions route into nearby discovery or activity advice instead of the generic scope error. Added regression tests. API-only; deploy via Vercel, no native build required.
- 2026-06-17 — Ask OutdoorAdvisor now prefers the app's exact current-location forecast snapshot for local weather questions instead of flattening answers to the broader city. Home passes a compact cached WeatherKit/Open-Meteo snapshot plus a precise area label; if reverse geocoding only says a major city, Ask infers the nearest known sub-area sample point (for example DHA/Gulberg/Model Town in Lahore). The backend validates that client weather evidence is recent before using it, marks Forecast as cached in source status, and still refetches live evidence for destination/travel questions. JS/API-only; ship via OTA + Vercel, no native build required.
- 2026-06-15 — Travel now refreshes NHMP, PMD, and NDMA together whenever the Travel tab receives focus, and users can pull down to force an immediate refresh of all three official sources. This fixes stale in-memory NDMA/PMD results when returning to an already-open Travel tab. JS-only; ship via OTA, no native build required.
- 2026-06-15 — Fixed fresh generic NDMA advisories being silently hidden from Travel and the morning official brief. NDMA often publishes a new document with only the title “Weather Advisory,” which cannot yet be classified as heat, flood, rain, or storm from the listing page alone. Newly published NDMA advisories are now promoted into the national watch for their first 72 hours even when the title is generic; old generic advisories still age out, and location-targeted push remains disabled unless affected regions can be matched. Added focused NDMA freshness/classification tests. Server-data-only behavior; no native build required.
- 2026-06-14 — Rebuilt premium Ask OutdoorAdvisor around question-specific planning rather than repeated current-condition fallbacks. Added deterministic intent/activity/time-window parsing (simple weather, activity advice, nearby discovery, destination trip), follow-up context inheritance, camping/hiking/mountain vocabulary, exposed-activity rain/wind thresholds, and specialized fallback structures. Forecast evidence now supports up to ten days; destination trips request a traffic-aware Google Routes route, sample weather along its actual encoded polyline when available, and continue cross-checking NHMP/PMD/NDMA. Places discovery now returns richer Google Maps candidates and can search broadly for mountain/camping requests, with forecast summaries for top options. The Ask sheet sends prior context for follow-ups, includes camping/Skardu example prompts, displays structured Journey/Destination/Plan sections, and labels Google route evidence. JS/API-only; no native build required.
- 2026-06-14 — Fixed the intermittent third/mixed theme seen after app launch in Auto mode. Root cause: React Native's `useColorScheme()` could initially return `null`, which ThemeContext interpreted as Dark for gradients while iOS `DynamicColorIOS` tokens independently resolved against the phone's actual Light appearance. ThemeContext now hydrates the saved preference, applies it to native `Appearance`, reads the native scheme directly, and waits for that synchronization before rendering themed screens. Explicit Light/Dark/Auto changes also update native appearance before React state, preventing split-theme frames. JS-only; ship via OTA, no native build required.
