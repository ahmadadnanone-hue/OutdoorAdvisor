...
## Recent Changes
- 2026-06-17 — Fixed Home sometimes showing only broad city names such as `Lahore` even when precise GPS coordinates were available. Added a shared precise-location helper that upgrades broad city/Pakistan labels to the nearest known sub-area from the AQI area grid (within 12 km), applies it to fresh and cached device locations, and reuses the same helper for Ask OutdoorAdvisor. Existing cached `Lahore, Pakistan` device labels are corrected on app launch without waiting for cache expiry. JS-only; ship via OTA, no native build required.
