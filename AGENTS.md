...
- The authenticated cron endpoint `/api/push?action=cron` is live in production. The GitHub Actions scheduler file was removed from the latest committed state because the current Git remote token cannot push workflow files without `workflow` scope, so timed automation still needs to be restored later with higher-scope GitHub credentials
...
- Server alert engine now covers: PMD severe/extreme CAP alerts, severe AQI threshold alerts, wind alerts, thunderstorm alerts, rain alerts (heavy and light), and morning summaries. Weather condition checks use WeatherKit as primary source (via `/api/weatherkit` Vercel proxy) with Open-Meteo as fallback. Remaining hardening follow-ups: NHMP closure/fog, extreme heat, smog season, pollen, route-specific alerts, and delivery dashboard
